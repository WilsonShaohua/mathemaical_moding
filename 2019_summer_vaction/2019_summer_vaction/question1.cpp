...
int main() {
	...
	/**建立不同维修级别工序之间顺序关系
	  * 将维修级别信息存储在一个二维数组中
	  * 行：表示不同的级别第零行对应维修级别I，第一行对应维修级别二，依次类推至第4行对应维修级别V
	  * 列：表示维修工序，第一列表示工序a，第二列表示工序b，依次类推至第六列对应工序e，
	  * 每行每列的值初始化为-1。
	  * 当第i行，第j列的值指向下一工序的列下标，第零列指向对应维修级别的第一维修工序
	  * 某一行的值为默认值时，表示该工序不在当前维修级别中或在当前维修级别中的最后一道维修工序
	  */
	 ...
	 *建立以train_id为唯一键值，maintenance_level的行下标为对应值的map映射
	 */
	...
	while (true) {
		...
		for (int i = 0; i < numbers; i++) {
			...
			while (!factories[i].empty() && f.end_work_time.cmp(now_time)) {
				...
				//下一维修工序不为结束工序（值：-1），则加入下一维修工序等待队列中
				if (process != -1) {
	...
	cout << "编号 类别 到达时间 A车间编号 A开始 A结束   B车间编号  B开始  B结束   C车间编号 C开始  C结束   D车间编号  D开始  D结束  E车间编号 E开始  E结束" << endl;
